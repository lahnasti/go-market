...
func (db *DBstorage) MakePurchase(purchase models.Purchase) (int, error) {
	...
	updateQuery := `UPDATE products SET quantity = quantity - $1 WHERE id = $2 AND quantity >= $1`
	...
	insertQuery := `INSERT INTO purchases (user_id, product_id, quantity) VALUES ($1, $2, $3) RETURNING id`
...
func (db *DBstorage) GetUserPurchases(userID int) ([]models.Purchase, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&purchase.ID, &purchase.UserID, &purchase.ProductID, &purchase.Quantity); err != nil {
			return nil, err
		}
